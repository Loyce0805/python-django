...
# Kernel trick(저차원을 고차원으로 변형) 기법을 사용함으로써 선형 분류 뿐만 아니라 비선형 분류도 할 수 있다.
# https://cafe.daum.net/flowlife/SBU0/17
...
plt.scatter(model.support_vectors_[:, 0], model.support_vectors_[:, 1], s=300, alpha=0.3)  # model.support_vectors로 support_vectors 값 얻을 수 있다.
...
plt.legend()
plt.title("SVM 예측 결과")
plt.show()  # 여기선 유클리디안 거리계산법을 쓴다 맨하튼 거리계산법 잘 안쓴다.
